from Nnetworks_MADDPGv2_flowV1 import CriticNetwork, ActorNetwork
import torch
from copy import deepcopy
from torch.optim import Adam
from memory_MADDPGv2_flowV1 import ReplayMemory, Experience
from random_process_MADDPGv2_flowV1 import OrnsteinUhlenbeckProcess
from torch.autograd import Variable
import os
import torch.nn as nn
import numpy as np
import torch as T
from utils_MADDPGv2_flowV1 import device
import csv
scale_reward = 0.01
...
class MADDPG:
    ...
    def choose_action(self, state, episode, env, noisy=True):
        ...
        for i in range(self.n_agents):
            ...
            actions[i, :] = act
        # make sure order don't mess up.
        # pair the output action with the agent's index in order.
        # "actions" always in order, the 1st element correspond to 1st agent in the env.all_agents
        In_order_action = actions.data.cpu().numpy()
        action_out = dict(zip(env.all_agents.keys(), In_order_action))

        self.steps_done += 1
        return action_out

    def get_scaling_factor(self, episode, drop_point=12000, start_scale=0.5, end_scale=0.03):  # start_scale=1 this is the original
