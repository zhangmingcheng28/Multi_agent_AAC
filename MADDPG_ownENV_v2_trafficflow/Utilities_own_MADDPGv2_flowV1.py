...
def compute_potential_conflict(pc_list, cur_drone_pos, cur_drone_vel, cur_drone_protRad, cur_neigh_pos, cur_neigh_vel,
    ...
    if all(n_p is None for n_p in cur_neigh_pos) and all(n_v is None for n_v in cur_neigh_vel):
        return pc_list  # this is for getting potential conflict when there are neighbour detected at first place
