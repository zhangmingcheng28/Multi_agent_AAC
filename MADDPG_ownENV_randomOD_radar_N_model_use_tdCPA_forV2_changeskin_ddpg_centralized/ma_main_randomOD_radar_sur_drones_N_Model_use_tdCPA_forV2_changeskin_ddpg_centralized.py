...
import pickle
import wandb
from parameters_randomOD_radar_sur_drones_N_Model_use_tdCPA_forV2_changeskin_ddpg_centralized import initialize_parameters
from maddpg_agent_randomOD_radar_sur_drones_N_Model_use_tdCPA_forV2_changeskin_ddpg_centralized import MADDPG
from utils_randomOD_radar_sur_drones_N_Model_use_tdCPA_forV2_changeskin_ddpg_centralized import *
...
def main(args):
    ...
    fully_observable_single_ddpg = True
    # fully_observable_single_ddpg = False
    ...
    if args.algo == "maddpg":
        model = MADDPG(actor_dim, critic_dim, n_actions, actor_hidden_state, gru_history_length, n_agents, args, criticNet_lr, actorNet_lr, args.gamma, args.tau, full_observable_critic_flag, fully_observable_single_ddpg, use_GRU_flag, use_single_portion_selfATT, use_selfATT_with_radar, use_allNeigh_wRadar, own_obs_only, env.normalizer, use_nearestN_neigh_wRadar, device)
    ...
    while episode < args.max_episodes:  # start of an episode
        ...
        while True:  # start of a step
            if args.mode == "train":
                ...
                action, step_noise_val, cur_actor_hiddens, next_actor_hiddens = model.choose_action(norm_cur_state, total_step, episode, step, eps_end, noise_start_level, cur_actor_hiddens, use_allNeigh_wRadar, use_selfATT_with_radar, own_obs_only, use_nearestN_neigh_wRadar, fully_observable_single_ddpg, noisy=noise_flag, use_GRU_flag=use_GRU_flag)  # noisy is false because we are using stochastic policy
                ...
                if len(gru_history) >= gru_history_length:
                    ...
                    if fully_observable_single_ddpg:
                        rw_tensor = torch.tensor(np.array(reward_aft_action), device=device).unsqueeze(1)
                    else:
                    ...
                    if full_observable_critic_flag:
                        ...
                        if fully_observable_single_ddpg:
                            done_tensor = torch.tensor(np.array(done_aft_action), device=device).unsqueeze(1)
                        else:
                    ...
                    if full_observable_critic_flag:
                        ...
                        model.memory.push(obs[0], obs[1], obs[2], ac_tensor, next_obs[0], next_obs[1], next_obs[2],
                                          rw_tensor, done_tensor, history_tensor, cur_actor_hiddens, next_actor_hiddens)
                    else:
                        ...
                        for i in range(len(one_agent_next_obs)):
                            ...
                            #                   cur_actor_hiddens[i, :], next_actor_hiddens[i,:])
                            model.memory[i].push(one_agent_obs[i][0], one_agent_obs[i][1], one_agent_obs[i][2], ac_tensor[i, :], one_agent_next_obs[i][0], one_agent_next_obs[i][1], one_agent_next_obs[i][2], rw_tensor[i], done_tensor[i], history_tensor[:,i,:],
                                              cur_actor_hiddens[i, :], next_actor_hiddens[i,:])
                ...
                # c_loss, a_loss, single_eps_critic_cal_record = model.update_myown_ddpg(episode, total_step, args.update_step, single_eps_critic_cal_record, transfer_learning, use_allNeigh_wRadar, use_selfATT_with_radar, use_nearestN_neigh_wRadar, wandb, full_observable_critic_flag, use_GRU_flag)  # last working learning framework
                c_loss, a_loss, single_eps_critic_cal_record = model.update_myown_single_ddpg(episode, total_step, args.update_step, single_eps_critic_cal_record, transfer_learning, use_allNeigh_wRadar, use_selfATT_with_radar, use_nearestN_neigh_wRadar, wandb, full_observable_critic_flag, use_GRU_flag)  # last working learning framework
