...
class ActorNetwork_combine(nn.Module):
    def __init__(self, actor_dim, n_actions):  # actor_obs consists of three parts 0 = own, 1 = own grid, 2 = surrounding drones
        super(ActorNetwork_combine, self).__init__()

        self.own_fc = nn.Sequential(nn.Linear(actor_dim[0], 128), nn.ReLU())
        self.own_grid = nn.Sequential(nn.Linear(actor_dim[1], 256), nn.ReLU())
        self.merge_feature = nn.Sequential(nn.Linear(128+256, 512), nn.ReLU())
        self.act_out = nn.Sequential(nn.Linear(512, n_actions), nn.Tanh())

    def forward(self, cur_state):
        own_obs = self.own_fc(cur_state[0])
        own_grid = self.own_grid(cur_state[1])
        merge_obs_grid = torch.cat((own_obs, own_grid), dim=2)
        merge_feature = self.merge_feature(merge_obs_grid)
        out_action = self.act_out(merge_feature)
        return out_action
...
class critic_combine(nn.Module):
    def __init__(self, critic_obs, n_actions):
        super(critic_combine, self).__init__()
        self.SA_fc = nn.Sequential(nn.Linear(critic_obs[0]+n_actions, 256), nn.ReLU())
        self.grid_fc = nn.Sequential(nn.Linear(critic_obs[1], 256), nn.ReLU())
        self.merge_fc_grid = nn.Sequential(nn.Linear(256+256, 512), nn.ReLU())
        self.out_feature_q = nn.Sequential(nn.Linear(512, 1))

    def forward(self, single_state, single_action):
        obsWaction = torch.cat((single_state[0], single_action), dim=2)
        combine_obsWaction = self.SA_fc(obsWaction)
        combine_grid = self.grid_fc(single_state[1])
        combine_obsWact_grid = torch.cat((combine_obsWaction, combine_grid), dim=2)
        merge_feature = self.merge_fc_grid(combine_obsWact_grid)
        q = self.out_feature_q(merge_feature)
        return q
