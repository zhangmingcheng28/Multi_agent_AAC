...
from Nnetworks_randomOD_radar_sur_drones_N_Model_use_tdCPA_forV2_changeskin_ddpg_centralized import *
from Utilities_own_randomOD_radar_sur_drones_N_Model_use_tdCPA_forV2_changeskin_ddpg_centralized import *
import torch
from copy import deepcopy
from torch.optim import Adam
from memory_randomOD_radar_sur_drones_N_Model_use_tdCPA_forV2_changeskin_ddpg_centralized import ReplayMemory, Experience
...
class MADDPG:
    def __init__(self, actor_dim, critic_dim, dim_act, actor_hidden_state_size, gru_history_length, n_agents,
                 args, cr_lr, ac_lr, gamma, tau, full_observable_critic_flag, fully_observable_single_ddpg, use_GRU_flag, use_single_portion_selfATT,
        ...
        if full_observable_critic_flag:
            ...
            if use_GRU_flag:
            ...
            elif fully_observable_single_ddpg:
                self.actors = ActorNetwork_combine(actor_dim, dim_act)
                ...
                # use multiple model for multiple agent
                self.actors = [ActorNetwork_TwoPortion(actor_dim, dim_act) for _ in
        ...
        if full_observable_critic_flag:
            ...
            if use_GRU_flag:
                ...
                self.critics = critic_combine(critic_dim, dim_act)
            elif use_single_portion_selfATT:
                ...
                self.critics = [
                    critic_single_TwoPortion(critic_dim, n_agents, dim_act, gru_history_length, actor_hidden_state_size)
        ...
        self.memory = ReplayMemory(args.memory_length)  # for one model, multiple agent
        # self.memory = [ReplayMemory(args.memory_length) for _ in range(n_agents)]  # for multiple model, multiple agent
    ...
    def update_myown_single_ddpg(self, i_episode, total_step_count, UPDATE_EVERY, single_eps_critic_cal_record, transfer_learning, use_allNeigh_wRadar, use_selfATT_with_radar, use_nearestN_neigh_wRadar, wandb=None, full_observable_critic_flag=False, use_GRU_flag=False):
        ...
        if full_observable_critic_flag:
            ...
            if use_GRU_flag:
                ...
                # non_final_next_actions = self.actors_target([non_final_next_states_actorin[0], non_final_next_states_actorin[2]])
                # non_final_next_actions = self.actors_target[agent]([non_final_next_states_actorin[0], non_final_next_states_actorin[2]])
                non_final_next_actions = self.actors_target([non_final_next_states_actorin[0], non_final_next_states_actorin[2]])
        ...
        if full_observable_critic_flag:
            ...
            if use_GRU_flag:
                ...
                # current_Q = self.critics[agent]([stacked_elem_0, stacked_elem_2], action_batch)
                current_Q = self.critics([stacked_elem_0, stacked_elem_2], action_batch)
                # current_Q = self.critics([stacked_elem_0, stacked_elem_2], action_batch)
        ...
        with T.no_grad():
            ...
            if full_observable_critic_flag:
                ...
                if use_GRU_flag:
                    ...
                    # next_target_critic_value = self.critics_target[agent]([next_stacked_elem_0, next_stacked_elem_2],non_final_next_actions).squeeze()
                    next_target_critic_value = self.critics_target([next_stacked_elem_0, next_stacked_elem_2],non_final_next_actions)
        ...
        if full_observable_critic_flag:
            if isinstance(self.actors, list):
                ...
                action_i = []
                for i in range(self.n_agents):
                    one_agent_act = self.actors[agent](
                        [stacked_elem_0_combine[:,i,:], stacked_elem_1_combine[:,i,:], stacked_elem_2_combine[:,i,:]])
                    action_i.append(one_agent_act)
                action_i = torch.stack(action_i, dim=1)
            ...
            if use_GRU_flag:
                ...
                # action_i = self.actors[agent]([stacked_elem_0, stacked_elem_2])
                action_i = self.actors([stacked_elem_0, stacked_elem_2])
                ac = action_i
                # ac = action_i.squeeze(0)  # replace the actor from self.actors[agent] into action batch
        ...
        if full_observable_critic_flag:
            ...
            if use_GRU_flag:
                ...
                # actor_loss = - self.critics([stacked_elem_0, stacked_elem_2], ac).mean()
                # actor_loss = - self.critics[agent]([stacked_elem_0, stacked_elem_2], ac).mean()
                actor_loss = - self.critics([stacked_elem_0, stacked_elem_2], ac).mean()
    ...
    def update_myown_ddpg(self, i_episode, total_step_count, UPDATE_EVERY, single_eps_critic_cal_record, transfer_learning, use_allNeigh_wRadar, use_selfATT_with_radar, use_nearestN_neigh_wRadar, wandb=None, full_observable_critic_flag=False, use_GRU_flag=False):

        self.train_num = i_episode

        if len(self.memory) <= self.batch_size:
        # if len(self.memory[0]) <= self.batch_size:  # for multi-agent multi-model
        # if len(self.memory) < self.memory.capacity:
        # if True:
            return None, None, single_eps_critic_cal_record
        # print("learning starts")
        ...
        # for agent in range(var_iteration):
        for agent in range(self.n_agents):  # for multi-agent multi-model
            if full_observable_critic_flag and isinstance(self.actors, list):
                if agent == 0:
                    transitions = self.memory.sample(self.batch_size)
                    batch = Experience(*zip(*transitions))
            ...
            with T.no_grad():
                ...
                if full_observable_critic_flag:
                    ...
                    if use_GRU_flag:
                        ...
                        next_target_critic_value = self.critics_target[agent]([next_stacked_elem_0, next_stacked_elem_2],
                            non_final_next_actions).squeeze()
    ...
    def choose_action(self, state, cur_total_step, cur_episode, step, mini_noise_eps, noise_start_level, actor_hiddens, use_allNeigh_wRadar, use_selfATT_with_radar, own_obs_only, use_nearestN_neigh_wRadar, fully_observable_single_ddpg, noisy=True, use_GRU_flag=False):
        ...
        if fully_observable_single_ddpg:
            if len(obs.shape) == 2 and len(obs_grid.shape) == 2:
                obs = obs.unsqueeze(0)
                obs_grid = obs_grid.unsqueeze(0)
            act = self.actors([obs, obs_grid])
            if len(act.shape) == 3:
                act = act.squeeze(0)  # remove batch information
            self.var = [
                self.get_custom_linear_scaling_factor(cur_episode, mini_noise_eps, noise_start_level)
                for _ in range(self.n_agents)
            ]
            if noisy:
                noise_value = [np.random.randn(2) * self.var[i] for i in range(self.n_agents)]
                act = [
                    act[i] + torch.from_numpy(noise_value[i]).to(self.device)
                    for i in range(self.n_agents)
                ]
            # actions = [torch.clamp(a, -1.0, 1.0) for a in act]  # when using stochastic policy, we are not require to clamp again.
            actions[:] = torch.stack([torch.clamp(a, -1.0, 1.0) for a in act])
        else:
            for i in range(self.n_agents):
            ...
            for i in range(self.n_agents):
                ...
                if use_GRU_flag:
                    ...
                    # act = self.actors([sb.unsqueeze(0), sb_grid.unsqueeze(0)])  # for one model multiple agent
                    act = self.actors[i]([sb.unsqueeze(0), sb_grid.unsqueeze(0)])  # for multiple model multiple agent
