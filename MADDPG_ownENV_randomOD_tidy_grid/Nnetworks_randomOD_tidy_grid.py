...
class ActorNetwork(nn.Module):
    def __init__(self, actor_dim, n_actions):  # actor_obs consists of three parts 0 = own, 1 = own grid, 2 = surrounding drones
        ...
        self.own_fc_lay2 = nn.Sequential(nn.Linear(64+64, 64), nn.ReLU(),
    ...
    def forward(self, state):
        ...
        own_grid = self.own_grid(state[1])
        own_combine = torch.concat((own_e, own_grid), dim=1)
        own_hidden = self.own_fc_lay2(own_combine)
...
class CriticNetwork(nn.Module):
    def __init__(self, critic_obs, n_agents, n_actions):
        ...
        # critic_obs[2] is sum of all other agent's observation
        ...
        self.sum_combine_fc = nn.Sequential(nn.Linear(256+256+(n_agents*n_actions), 256), nn.ReLU())
    ...
    def forward(self, state, actor_actions):  # state[0] is sum of all agent's own observed states, state[1] is is sum of all agent's observed grid maps
        ...
        env_concat = torch.cat((sum_own, sum_grid, actor_actions), dim=1)

        combine_state = self.sum_combine_fc(env_concat)

        q = self.combine_all(combine_state)
        return q
