...
from agent_MADDPGv4 import Agent
import pandas as pd
import math
import numpy as np
import os
...
import re
import time
from Utilities_own_MADDPGv4 import *
import torch as T
import torch
...
class env_simulator:
    ...
    def create_world(self, total_agentNum, n_actions, gamma, tau, target_update, largest_Nsigma, smallest_Nsigma, ini_Nsigma, max_xy, max_spd, acc_range):
        ...
        # config normalizer
        self.normalizer = NormalizeData(max_xy[0], max_xy[1], max_spd, acc_range)
        self.all_agents = {}
        ...
        # global_state = self.reset_world(show=0)  # not necessary
    ...
    def reset_world(self, show):  # set initialize position and observation for all agents
        ...
        overall_state, norm_overall_state = self.cur_state_norm_state_v4(agentRefer_dict)
        ...
        for agent_pos, agent_idx_string in agentRefer_dict.items():
            ...
            if np.linalg.norm(np.array(point_to_search) - np.array(agent_pos))<=search_distance:
                ...
                cur_agent.surroundingNeighbor[other_agent_idx] = np.array([self.all_agents[other_agent_idx].pos[0],
                                                                           ...
                                                                           self.all_agents[other_agent_idx].vel[0],
                                                                           self.all_agents[other_agent_idx].vel[1],
                                                                           self.all_agents[other_agent_idx].acc[0],
                                                                           self.all_agents[other_agent_idx].acc[1],
                                                                           self.all_agents[other_agent_idx].cur_pc_count,
                                                                           self.all_agents[other_agent_idx].goal[0][0],
                                                                           self.all_agents[other_agent_idx].goal[0][1]])
                # cur_agent.surroundingNeighbor[other_agent_idx] = self.all_agents[other_agent_idx]  # DON't move in the entire class, or else will eat a lot of CPU time when doing deepcopy()
    ...
    def cur_state_norm_state_v3(self, agentRefer_dict):
        ...
        # loop over all agent again to obtain each agent's detectable neighbor
        # second loop is required, because 1st loop is used to obtain all agent's surrounding agent and make sure their current total number of potential collision is populated
        # second loop is to generate the overall_state_p3 and norm_overall_state_p3.
        # If we infuse second loop into 1st loop, will have error because P3 need surround neighbour's current PC count
        # If we don't make sure current surround neighbour's current PC count is populated, their might be possibility that
        # current agent idx = 0, and its has neighbor idx=3, but due to the use of for loop, the current PC count's number is not populated yet.
        # first loop
        for agentIdx, agent in self.all_agents.items():
            pc_current = []  # current total number of potential conflicts for each agent, so must be cleared for every new agent
            # get current agent's name in term of integer
            match = re.search(r'\d+(\.\d+)?', agent.agent_name)
            if match:
                agent_idx = int(match.group())
            ...
            # --------- obtain the total number of potential conflicts at current state -----------------
            for neigh_keys in self.all_agents[agentIdx].pre_surroundingNeighbor:
                pc_current = compute_potential_conflict(pc_current, agent.pos, agent.vel,
                                                      agent.protectiveBound, self.all_agents[neigh_keys].pos,
                                                      self.all_agents[neigh_keys].vel,
                                                      self.all_agents[neigh_keys].protectiveBound, neigh_keys)
            # load in current potential count at current state
            agent.cur_pc_count = len(pc_current)
            # -------- end of obtain the total number of potential conflicts at current state ------------

            agent_own = np.array(
                [agent.pos[0], agent.pos[1], agent.goal[0][0] - agent.pos[0], agent.goal[0][1] - agent.pos[1],
                 agent.vel[0], agent.vel[1], agent.acc[0], agent.acc[1], agent.cur_pc_count])
            # populate normalized agent_own
            # norm_agent_own = []
            norm_pos = self.normalizer.nmlz_pos([agent.pos[0], agent.pos[1]])
            ...
            norm_agent_own = np.array(list(norm_pos + norm_G_diff + norm_vel + norm_acc)+[agent.cur_pc_count])
        ...
        for agentIdx, agent in self.all_agents.items():
            ...
            if len(agent.surroundingNeighbor) > 0:  # meaning there is surrounding neighbors around the current agent
                for other_agentIdx, other_agent in agent.surroundingNeighbor.items():
                    if other_agentIdx != agentIdx:
                        ...
                        surround_agent = np.array([[other_agent[0] - agent.pos[0],
                                                    ...
                                                    other_agent[4], other_agent[5],
                                                    other_agent[6]]])

                        norm_pos_diff = self.normalizer.nmlz_pos_diff(
                            [other_agent[0] - agent.pos[0], other_agent[1] - agent.pos[1]])
                        # norm_pos_diff = self.normalizer.nmlz_pos_diff(
                        #     [other_agent.pos[0] - agent.pos[0], other_agent.pos[1] - agent.pos[1]])

                        norm_G_diff = self.normalizer.nmlz_pos_diff(
                            [other_agent[-2] - other_agent[0], other_agent[-1] - other_agent[1]])
                        # norm_G_diff = self.normalizer.nmlz_pos_diff(
                        #     [other_agent.goal[0][0] - other_agent.pos[0], other_agent.goal[0][1] - agent.pos[1]])

                        norm_vel = self.normalizer.nmlz_vel([other_agent[2], other_agent[3]])

                        norm_acc = self.normalizer.nmlz_acc([other_agent[4], other_agent[5]])

                        norm_surround_agent = np.array([list(norm_pos_diff + norm_G_diff + norm_vel + norm_acc) + [other_agent[6]]])

                        # norm_vel = self.normalizer.nmlz_vel([other_agent.vel[0], other_agent.vel[1]])
                        #
                        # norm_acc = self.normalizer.nmlz_acc([other_agent.acc[0], other_agent.acc[1]])
                        #
                        # norm_surround_agent = np.array([list(norm_pos_diff + norm_G_diff + norm_vel + norm_acc) + [other_agent.cur_pc_count]])
                ...
                norm_overall_state_p3.append([np.zeros((1, 9))])  # normalize state is it ok to fill with 0?
    ...
    def cur_state_norm_state_v4(self, agentRefer_dict):
        overall = []
        norm_overall = []
        # prepare for output states
        overall_state_p1 = []
        ...
        for agentIdx, agent in self.all_agents.items():
            ...
            # nx2 feature vector for observable space
            self.all_agents[agentIdx].observableSpace = self.current_observable_space(self.all_agents[agentIdx])
    ...
    def current_observable_space(self, cur_agent):
        ...
        grid_obs = np.zeros((len(no_sorted_polySet), 2))  # nx2
        for poly_idx, poly in enumerate(no_sorted_polySet):
            if self_circle.intersects(poly):
                # currentObservableState.append(occupied_drone_val)
                grid_obs[poly_idx, 0] = occupied_drone_val
                grid_obs[poly_idx, 1] = shapely.distance(poly.centroid, Point(t_x, t_y))
                continue
            if poly in self.world_map_2D_polyList[0][0]:
                currentObservableState.append(occupied_building_val)
                grid_obs[poly_idx, 0] = occupied_building_val
                grid_obs[poly_idx, 1] = shapely.distance(poly.centroid, Point(t_x, t_y))
            else:
                currentObservableState.append(non_occupied_val)
                grid_obs[poly_idx, 0] = non_occupied_val
                grid_obs[poly_idx, 1] = shapely.distance(poly.centroid, Point(t_x, t_y))
    ...
    def get_step_reward_5_v3(self, current_ts, step_reward_record):  # this is for individual drones, current_ts = current time step
        ...
        for drone_idx, drone_obj in self.all_agents.items():
            ...
            before_dist_hg = np.linalg.norm(drone_obj.pre_pos - drone_obj.goal[0])
            after_dist_hg = np.linalg.norm(drone_obj.pos - drone_obj.goal[0])  # distance to goal after action
            delta_hg = before_dist_hg - after_dist_hg
            ...
            # exceed bound condition, don't use current point, use current circle or else will have condition that
                ...
                if len(drone_obj.goal) > 1:  # meaning the current agent has more than one target/goal
                    ...
                    # normal_step_rw = crossCoefficient*cross_track_error + goalCoefficient * delta_hg + alive_penalty
                    normal_step_rw = crossCoefficient*cross_track_error + dominoCoefficient*dominoTerm + goalCoefficient * delta_hg + alive_penalty
                ...
                # step_reward =crossCoefficient*cross_track_error + goalCoefficient * delta_hg + alive_penalty
                step_reward =crossCoefficient*cross_track_error + dominoCoefficient*dominoTerm + goalCoefficient * delta_hg + alive_penalty
        ...
        if len(reward) != len(self.all_agents):
            print("Please Debug, the total number of rewards don't equals to total number of agents")
        shared_reward = np.array(sum(reward), dtype=float)
        reward = [shared_reward] * len(self.all_agents)
    ...
    def step(self, actions, current_ts):
        ...
        agentRefer_dict = {}  # A dictionary to use agent's current pos as key, their agent name (idx) as value
        # we use 1.4 here, because the time-step for the simulation is 0.5 sec.
        # hence, 1.4 here is equivalent to the acceleration of 2.8m/s^2 for range of one axis acceleration range
        coe_a = 1.4  # coe_a is the coefficient of action is 1.4 because our time step is 0.5 sec
        #This is for acceleration in one axis, when both x & y axis have range of -1.4 to 1.4, the combined acceleration can be around -4 to 4 m/s^2, that is inline with the DJI's specification.
        ...
        next_state, next_state_norm = self.cur_state_norm_state_v4(agentRefer_dict)
