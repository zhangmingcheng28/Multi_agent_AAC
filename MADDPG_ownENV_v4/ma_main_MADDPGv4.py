...
import pickle
import wandb
from parameters_MADDPGv4 import initialize_parameters
from maddpg_agent_MADDPGv4 import MADDPG
from utils_MADDPGv4 import *
...
def main(args):
    ...
    actor_dim = [9, 2, 9]  # dim host, maximum dim grid, dim other drones
    critic_dim = [9, 2, 9]
    n_actions = 2
    acc_range = [-2.8, 2.8]  # [min, max]
    ...
    while episode < args.max_episodes:
        ...
        while True:
            if args.mode == "train":
                ...
                action = model.choose_action(norm_cur_state, episode, noisy=True)  # did not update anything only generate next action
                ...
                if args.algo == "maddpg" or args.algo == "commnet":
                    ...
                    # for elementIdx, element in enumerate(cur_state):
                        if elementIdx == 0:  # 1st element in the state only
                        # if elementIdx != len(cur_state)-1:  # meaning is not the last element
                            ...
                            # surrounding agents & surrounding grids for the agent
                            sur_agents = []
                            for each_agent_list in element:
                                if isinstance(each_agent_list, np.ndarray):
                    ...
                    # for elementIdx, element in enumerate(cur_state):
                        if elementIdx == 0:   # 1st element in the next state only
                        # if elementIdx != len(cur_state)-1:  # meaning is not the last element
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', default=512, type=int)  # original is 512
