...
class ActorNetwork(nn.Module):
    def __init__(self, actor_dim, n_actions):  # actor_obs consists of three parts 0 = own, 1 = own grid, 2 = surrounding drones
        ...
        self.intrude_fc = nn.Sequential(nn.Linear(actor_dim[2], 128), nn.ReLU())
        ...
        # attention for grids
        self.grid_k = nn.Linear(128, 128, bias=False)
        self.grid_q = nn.Linear(128, 128, bias=False)
        self.grid_v = nn.Linear(128, 128, bias=False)
    ...
    def forward(self, state):
        ...
        grid_e = self.own_grid_fc(state[1])

        # mask attention embedding for grids
        grid_q = self.grid_q(own_e)
        grid_k = self.grid_k(grid_e)
        grid_v = self.grid_v(grid_e)
        mask = state[1].mean(axis=2, keepdim=True).bool()
        score = torch.bmm(grid_k, grid_q.unsqueeze(axis=2))
        score_mask = score.clone()  # clone操作很必要
        score_mask[~mask] = float('-inf')  # 不然赋值操作后会无法计算梯度

        alpha = F.softmax(score_mask / np.sqrt(grid_v.size(-1)), dim=1)  # we use dim=1 here because we need to get attention of each sequence in K towards all hidden vector of q in each batch.
        alpha_mask = alpha.clone()
        alpha_mask[~mask] = 0
        grid_v_att = torch.sum(grid_v * alpha_mask, axis=1)

        # mask attention embedding for surrounding agent
        # value key is extracted from surround agent encoding because our subject is surround agent, we want to extract a fixed length feature representation of surrounding agents
        # under the influence of the host agent encoding.
        q = self.q(own_e)
        k = self.k(x_e)
        v = self.v(x_e)
        ...
        concat = torch.cat((own_e, v_att, grid_v_att), dim=1)
        # concat = torch.cat((own_e, own_grid), dim=1)
...
class CriticNetwork_0724(nn.Module):
    def __init__(self, critic_obs, n_agents, n_actions):
        ...
        self.action_encoding = nn.Sequential(nn.Linear(n_agents * n_actions, 32), nn.ReLU())
        # for surrounding agents' encoding, for each agent, we there are n-neighbours, each neighbour is represented by
        # a vector of length = 6. Before we put into an experience replay, we pad it up to max_num_neigh * 6 array.
        # so, one agent will have an array of max_num_neigh * 6, after flatten, then for one batch, there are a total of
        # n_agents exist in the airspace, therefore, the final dimension will be max_num_neigh * 6 * max_num_neigh.
        ...
        # critic attention for overall sur_grid with overall own_state
        self.single_grid_k = nn.Linear(128, 128, bias=False)
        self.single_grid_q = nn.Linear(128, 128, bias=False)
        self.single_grid_v = nn.Linear(128, 128, bias=False)
        #
        # self.n_heads = 3
        # self.single_head_dim = int((256+256+256) / self.n_heads)
        # self.com_k = nn.Linear(self.single_head_dim, self.single_head_dim, bias=False)
        ...
        # self.combine_env_fc = nn.Sequential(nn.Linear(256+128, 128), nn.ReLU())
        # self.combine_env_fc = nn.Sequential(nn.Linear((n_agents*128)+(n_agents*128), 64), nn.ReLU())
        self.combine_env_fc = nn.Sequential(nn.Linear((n_agents*128)+(n_agents*128)+(n_agents*128), 1024), nn.ReLU(), nn.Linear(1024, 128), nn.ReLU())

        # self.combine_all = nn.Sequential(nn.Linear(64+n_agents * n_actions, 64), nn.ReLU(), nn.Linear(64, 1))
        self.combine_all = nn.Sequential(nn.Linear(128+32, 64), nn.ReLU(), nn.Linear(64, 1))
    ...
    def forward(self, state, actor_actions):  # state[0] is sum of all agent's own observed states, state[1] is is sum of all agent's observed grid maps
        # pre-process, compute attention for every agent based on their surrounding agents
        attention_all_agent = []
        attention_all_agent_grid = []
        own_all_agent = []
        for one_agent_batch_own, one_agent_batch_grid, one_agent_batch_surr in zip(*state):  # automatically loop over 5 times
            single_own_out = self.single_own_fc(one_agent_batch_own)

            single_grid_out = self.single_grid_fc(one_agent_batch_grid)
            single_grid_q = self.single_grid_q(single_own_out)
            single_grid_k = self.single_grid_k(single_grid_out)
            single_grid_v = self.single_grid_v(single_grid_out)
            mask = one_agent_batch_grid.mean(axis=2, keepdim=True).bool()
            score = torch.bmm(single_grid_k, single_grid_q.unsqueeze(axis=2))
            score_mask = score.clone()  # clone操作很必要
            score_mask[~mask] = float('-inf')  # 不然赋值操作后会无法计算梯度
            alpha = F.softmax(score_mask / np.sqrt(single_grid_k.size(-1)), dim=1)  # we use dim=1 here because we need to get attention of each sequence in K towards all hidden vector of q in each batch.
            alpha_mask = alpha.clone()
            alpha_mask[~mask] = 0
            grid_v_att = torch.sum(single_grid_v * alpha_mask, axis=1)
            attention_all_agent_grid.append(grid_v_att)
        ...
        sum_grid = torch.stack(attention_all_agent_grid).transpose(0, 1)
        sum_grid = sum_grid.reshape(sum_grid.shape[0], -1)
        ...
        # env_concat = torch.cat((sum_att, sum_grid), dim=1)
        # env_concat = torch.cat((sum_own, sum_grid), dim=1)
        env_concat = torch.cat((sum_own, sum_grid, sum_att), dim=1)

        env_encode = self.combine_env_fc(env_concat)
        act_encode = self.action_encoding(actor_actions)
        entire_comb = torch.cat((env_encode, act_encode), dim=1)
        # entire_comb = torch.cat((sum_own_e, actor_actions), dim=1)
        q = self.combine_all(entire_comb)
        return q
