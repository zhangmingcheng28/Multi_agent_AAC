...
def update_cloud_target(cloud, cloud_path, corridor_radius=5, target_radius=1):
    traj_line = LineString([cloud.pre_pos, cloud.pos])
    corridor = traj_line.buffer(corridor_radius, cap_style=1)  # round caps
    target_zone = cloud.previous_target.buffer(target_radius)
    current_index = cloud_path.index(cloud.previous_target)
    if corridor.intersects(target_zone):
        # Move to the left (clockwise)
        next_index = (current_index + 1) % len(cloud_path)
        cloud.previous_target = cloud_path[next_index]
    target_out = np.array(cloud.previous_target.coords[0])
    return target_out, cloud
...
def generate_start_position(start_zones):
    """
    Generate a random [x, y] start position within one of the allowed start_zones.

    Args:
        start_zones: list or tuple of zones, where each zone is (x_min, x_max, y_min, y_max)

    Returns:
        A list [x, y] representing the sampled start position
    """
    # Randomly choose one of the start zones
    selected_region = random.choice(start_zones)
...
def generate_random_end_pos(bounds, no_fly_zones, random_start_pos, runway_bound: Polygon):
    xmin, xmax, ymin, ymax = bounds
    runway_xmin, _, runway_xmax, _ = runway_bound[0].bounds  # bounds = (minx, miny, maxx, maxy)

    x_start = random_start_pos[0]
    if runway_xmin <= x_start <= runway_xmax:
        raise ValueError(f"Start position x = {x_start} is inside the runway corridor [{runway_xmin}, {runway_xmax}]")

    # Initialize with full allowed area
    possible_regions = [(xmin, xmax, ymin, ymax)]

    # Subtract no-fly zones
    for no_fly_xmin, no_fly_xmax, no_fly_ymin, no_fly_ymax in no_fly_zones:
        new_regions = []
        for rxmin, rxmax, rymin, rymax in possible_regions:
            if rxmin < no_fly_xmax and rxmax > no_fly_xmin and rymin < no_fly_ymax and rymax > no_fly_ymin:
                if rxmin < no_fly_xmin:
                    new_regions.append((rxmin, no_fly_xmin, rymin, rymax))
                if rxmax > no_fly_xmax:
                    new_regions.append((no_fly_xmax, rxmax, rymin, rymax))
                if rymin < no_fly_ymin:
                    new_regions.append((max(rxmin, no_fly_xmin), min(rxmax, no_fly_xmax), rymin, no_fly_ymin))
                if rymax > no_fly_ymax:
                    new_regions.append((max(rxmin, no_fly_xmin), min(rxmax, no_fly_xmax), no_fly_ymax, rymax))
            else:
                new_regions.append((rxmin, rxmax, rymin, rymax))
        possible_regions = new_regions

    if not possible_regions:
        raise ValueError("No valid regions remain after excluding no-fly zones.")

    # Filter valid regions: end x must be on same side as start x
    if x_start < runway_xmin:
        filtered_regions = [r for r in possible_regions if r[1] <= runway_xmin]
    elif x_start > runway_xmax:
        filtered_regions = [r for r in possible_regions if r[0] >= runway_xmax]
    else:
        filtered_regions = []

    if not filtered_regions:
        raise ValueError("No valid regions remain on the same side of the runway as the start.")

    # Sample end position from allowed region
    selected_region = filtered_regions[np.random.randint(0, len(filtered_regions))]
    center_x = np.random.uniform(selected_region[0], selected_region[1])
    center_y = np.random.uniform(selected_region[2], selected_region[3])
...
def animate(frame_num, ax, env, trajectory_eachPlay):
    ...
    plt.xlabel("X axis")
    plt.ylabel("Y axis")
    # plot aerodrome area as a circle (radius 5, center at (20, 20))
    ax.add_patch(plt.Circle((20, 20), 5, facecolor='blue', edgecolor='black', alpha=0.5))
    # plot runway boundary
    for rw_obj in env.runway_bound:
        rw_x, rw_y = rw_obj.exterior.xy
        ax.plot(rw_x, rw_y, linestyle='--', linewidth=1, color='black')

    aircraft_svg_path = r'D:\MADDPG_2nd_jp\Aircraft_eVTOL.svg'  # Replace with your SVG path
    manned_aircraft_svg_path = r'D:\MADDPG_2nd_jp\Aircraft.svg'  # Replace with your SVG path
    plane_img = load_svg_image(aircraft_svg_path)
    manned_plane_img = load_svg_image(manned_aircraft_svg_path)
    ...
    #___________ display cloud __________
    ...
    for cloud_idx, cloud_agent in enumerate(env.cloud_config):
        if cloud_agent.agent_essence == 'go_aircraft':
        ...
        # ___add boundary circle for clouds _____
        matp_poly = shapelypoly_to_matpoly(cloud_poly, False, 'blue')  # the 3rd parameter is the edge color
        matp_poly.set_zorder(5)
        ax.add_patch(matp_poly)
    ...
    for a_idx, agent in enumerate(trajectory_eachPlay[frame_num]):
        ...
        #_______ draw agent prob line ______
        ...
        self_circle = Point(x, y).buffer(evtol_size, cap_style='round')
    ...
    #___________ go_around_aircraft ______________
    if frame_num == len(trajectory_eachPlay):     # last time step
        pre_pos = env.cloud_config[1].trajectory[frame_num-1].coords[0]
        cur_pos = env.cloud_config[1].trajectory[frame_num].coords[0]
    else:
        pre_pos = env.cloud_config[1].trajectory[frame_num].coords[0]
        cur_pos = env.cloud_config[1].trajectory[frame_num+1].coords[0]
    dx = cur_pos[0] - pre_pos[0]
    dy = cur_pos[1] - pre_pos[1]
    go_around_aircraft_heading = np.degrees(np.arctan2(dy, dx)) % 360
    go_around_aircraft_x = cur_pos[0]
    go_around_aircraft_y = cur_pos[1]
    aircraft_size = 3
    go_around_img_extent = [
        go_around_aircraft_x - aircraft_size,
        go_around_aircraft_x + aircraft_size,
        go_around_aircraft_y - aircraft_size,
        go_around_aircraft_y + aircraft_size
    ]
    transform = Affine2D().rotate_deg_around(go_around_aircraft_x, go_around_aircraft_y, go_around_aircraft_heading - 90) + ax.transData
    ax.imshow(manned_plane_img, extent=go_around_img_extent, zorder=10, transform=transform)
...
def view_static_traj(env, trajectory_eachPlay, save_path=None, max_time_step=None):
    aircraft_svg_path = r'D:\MADDPG_2nd_jp\Aircraft_eVTOL.svg'  # Replace with your SVG path
    go_aircraft_svg_path = r'D:\MADDPG_2nd_jp\Aircraft.svg'  # Replace with your SVG path
    plane_img = load_svg_image(aircraft_svg_path)
    go_plane_img = load_svg_image(go_aircraft_svg_path)
    os.environ["KMP_DUPLICATE_LIB_OK"] = "TRUE"
    matplotlib.use('TkAgg')
    fig, ax = plt.subplots(1, 1)
    colors = [
    ...
    # display go_around aircraft
    for cloud_idx, cloud_agent in enumerate(env.cloud_config):
        if cloud_agent.agent_essence == 'go_aircraft':
            # Extract x and y coordinates
            go_ac_x = [pt.x for pt in cloud_agent.trajectory]
            go_ac_y = [pt.y for pt in cloud_agent.trajectory]

            # Parameterized B-spline curve
            tck, u = interpolate.splprep([go_ac_x, go_ac_y], s=3)
            unew = np.linspace(0, 1.0, 200)
            x_spline, y_spline = interpolate.splev(unew, tck)
            # plt.plot(go_ac_x, go_ac_y, linestyle=':', linewidth=2, color='black')  # ':' = dotted line
            # plt.plot(go_ac_x, go_ac_y, 'o', color='gray', markersize=2, label='Original')
            plt.plot(x_spline, y_spline, '-', color='blue', label='Smoothed B-spline')

            if trajectory_idx == max_time_step - 1:

                # Final position with aircraft marker
                dx = cloud_agent.trajectory[-1].x - cloud_agent.trajectory[-2].x
                dy = cloud_agent.trajectory[-1].y - cloud_agent.trajectory[-2].y
                angle_rad = np.arctan2(dy, dx)
                heading = (270 + np.degrees(angle_rad)) % 360
                go_aircraft_bound = 2 # size of go-around aircraft icon
                img_extent = [
                    go_ac_x[-1] - go_aircraft_bound,
                    go_ac_x[-1] + go_aircraft_bound,
                    go_ac_y[-1] - go_aircraft_bound,
                    go_ac_y[-1] + go_aircraft_bound
                ]
                transform = Affine2D().rotate_deg_around(go_ac_x[-1], go_ac_y[-1], heading) + ax.transData
                ax.imshow(go_plane_img, extent=img_extent, zorder=10, transform=transform)

        else:
            continue

    # display UAM agent trajectory
    agent_size = 1.2
    for agentIDX, agent in env.all_agents.items():
        ...
        for trajectory_idx in range(max_time_step):
            ...
            if trajectory_idx == max_time_step - 1: # final position
                # Final position with aircraft marker
            ...
            if trajectory_idx == max_time_step - 1:  # final position
                #____________ plot start and end position marker ____________
                start_x, start_y = agent.ini_pos[0], agent.ini_pos[1]
                end_x, end_y = x, y
                # Start position: green triangle
                ax.plot(start_x, start_y, marker='^', color='green', markersize=7, label='Start')
                # End position: red star
                ax.plot(end_x, end_y, marker='*', color='red', markersize=7, label='End')

                #_________ Draw the protective boundary around the final position of agent
                # self_circle = Point(x, y).buffer(agent_size, cap_style='round')
                # grid_mat_SCir = shapelypoly_to_matpoly(self_circle, inFill=True, Edgecolor=None, FcColor='lightblue')
                # grid_mat_SCir.set_zorder(2)
                # grid_mat_SCir.set_alpha(0.9)
                if agent.ini_eta == None or agent.ini_eta < trajectory_idx:
                    # ax.add_patch(grid_mat_SCir)
                    plt.text(x+1, y+1, 'u'+str(agentIDX+1))
    ...
    plt.ylabel("Width (km)")
    legend_elements = [
        Line2D([0], [0], linestyle='--', color='gray', label='UAM trajectory'),  # dashed
        Line2D([0], [0], linestyle='-', color='blue', label='Aircraft go-around trajectory'),  # solid
        Line2D([0], [0], marker='^', color='green', linestyle='None', markersize=7, label='UAM start position'),
        Line2D([0], [0], marker='*', color='red', linestyle='None', markersize=7, label='UAM rejoin position'),
        Line2D([0], [0], linestyle='--', color='black', label='Runway protection zone')  # dashed box
    ]

    ax.legend(handles=legend_elements, loc='best')  # or choose a suitable location
