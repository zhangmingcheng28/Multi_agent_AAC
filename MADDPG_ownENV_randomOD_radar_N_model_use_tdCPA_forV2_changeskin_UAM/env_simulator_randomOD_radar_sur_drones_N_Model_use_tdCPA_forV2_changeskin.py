...
class env_simulator:
    ...
    def reset_world_change_skin(self, total_agentNum, full_observable_critic_flag, evaluation_by_fixed_ar, include_other_AC, use_nearestN_neigh_wRadar, N_neigh, args, show):  # set initialize position and observation for all agents
        ...
        # ____________________________________ start of env setting 40 x 40 _______________
        # ----------runway bound ----------
        self.runway_bound = [Polygon([(18, 10), (18, 30), (22, 30), (22, 10), (18, 10)])]
        # -------- start of add go around trajectories -----------
        go_0 = [20, 20, 20, 35, 5, 35, 5, 5, 20, 5, 20, 20]
        go_1 = [20, 20, 20, 5, 5, 5, 5, 35, 20, 35, 20, 20]
        go_2 = [20, 20, 20, 35, 35, 35, 35, 5, 20, 5, 20, 20]
        go_3 = [20, 20, 20, 5, 35, 5, 35, 35, 20, 35, 20, 20]
        go_AC = (go_0, go_1, go_2, go_3)
        # -------- END of add go around trajectories -----------
        ...
        # ___________ representation of could and aircraft _________________
        # ___ cloud_0 represent cloud ___
        cloud_0 = random.choice(clouds)
        # ___ cloud_1 represent go around aircraft ___
        cloud_1 = random.choice(go_AC)
        # cloud_1 = go_0

        all_clouds = [cloud_0, cloud_1]  # cloud out of the map
        # --------- end of manually assigned a fixed cloud OD config 4 graph-------------- #

        # -------- UAM start positions: within aerodrome_two sides -----------
        start_zone_1 = (15, 17, 15, 25)
        start_zone_2 = (23, 25, 15, 25)
        start_zones = (start_zone_1, start_zone_2)

        # -------- no spawn zones -----------
        no_spawn_zone = []
        aerodrome_no_spawn_zone = (10, 30, 10, 30)
        no_spawn_zone.append(aerodrome_no_spawn_zone)
        no_spawn_zone.append(start_zone_1)
        no_spawn_zone.append(start_zone_2)
        # ____________________________________ end of env setting 40 x 40 _______________

        # ____________________________________ start of env setting 50 x 50 _______________
        # #----------runway bound ----------
        # self.runway_bound = [Polygon([(23, 15), (23, 35), (27, 35), (27, 15), (23, 15)])]
        # # -------- start of add go around trajectories -----------
        # go_0 = [25, 25, 25, 45, 5, 45, 5, 5, 25, 5, 25, 25]
        # go_1 = [25, 25, 25, 5, 5, 5, 5, 40, 25, 45, 25, 25]
        # go_2 = [25, 25, 25, 45, 45, 45, 45, 5, 25, 5, 25, 25]
        # go_3 = [25, 25, 25, 5, 45, 5, 45, 45, 25, 45, 25, 25]
        # go_AC = (go_0, go_1, go_2, go_3)
        # # -------- END of add go around trajectories -----------
        #
        # # -------- start of add cloud -----------
        ...
        #
        # # -------- UAM start positions: within aerodrome_two sides -----------
        # start_zone_1 = (20, 22, 20, 30)
        # start_zone_2 = (28, 30, 20, 30)
        # start_zones = (start_zone_1, start_zone_2)
        #
        # # -------- no spawn zones -----------
        # no_spawn_zone = []
        # aerodrome_no_spawn_zone = (15, 35, 15, 35)
        # no_spawn_zone.append(aerodrome_no_spawn_zone)
        # no_spawn_zone.append(start_zone_1)
        # no_spawn_zone.append(start_zone_2)
        # ____________________________________ end of env setting 50 x 50 _______________
        ...
        cloud_config = []
        no_spawn_zone = []
        spawn_threshold_cloud = 5
        spawn_threshold_boundary = 5 # start or end position near boundary
        for cloud_idx, cloud_setting in enumerate(all_clouds):
            cloud_a = cloud_agent(cloud_idx)
            cloud_a.agent_essence_setting()
            cloud_a.pos = Point(cloud_setting[0], cloud_setting[1])
            cloud_a.ini_pos = cloud_a.pos
            cloud_a.cloud_actual_cur_shape = cloud_a.pos.buffer(cloud_a.radius)
            cloud_a.goal = Point(cloud_setting[2], cloud_setting[3])
            cloud_a.trajectory.append(cloud_a.pos)
            if len(cloud_setting) > 4:
                cloud_a.preset_traj = [Point(cloud_setting[i], cloud_setting[i + 1]) for i in range(0, len(cloud_setting), 2)]
                cloud_a.previous_target = cloud_a.preset_traj[1]
            cloud_config.append(cloud_a)
        ...
        for agentIdx in self.all_agents.keys():
            # Select random start/target indices (as before)
            random_start_index = random.randint(0, len(self.target_pool) - 1)
            numbers_left = list(range(0, random_start_index)) + list(
                range(random_start_index + 1, len(self.target_pool)))
            random_target_index = random.choice(numbers_left)

            # Sample a random start position from allowed zones
            random_start_pos = generate_start_position(start_zones)

            # If needed, ensure no overlap between starting drones
            if len(start_pos_memory) > 0:
                while len(start_pos_memory) < len(self.all_agents):
                    random_start_pos = generate_start_position(start_zones)
                    if all(np.linalg.norm(np.array(random_start_pos) - point) > self.all_agents[
                        agentIdx].protectiveBound * 3 # distance between a pair of UAM flight to avoid collion at step 0
                           for point in start_pos_memory):
                        break

            # random_end_pos = random.choice(self.target_pool[random_target_index])
            random_end_pos = generate_random_end_pos(self.bound, no_spawn_zone, random_start_pos, self.runway_bound)
    ...
    def cur_state_norm_state_v3(self, agentRefer_dict, full_observable_critic_flag, include_other_AC, use_nearestN_neigh_wRadar, N_neigh, args, evaluation_by_fixed_ar):
        ...
        for agentIdx, agent in self.all_agents.items():
            ...
            for point_deg, point_pos in st_points.items():
                ...
                # check if line intersect with runway_bound
                rw_nearest_intersection_point = None
                rw_nearest_distance = math.inf
                for rw_obj in self.runway_bound:
                    rw_boundary = rw_obj.boundary
                    if cur_host_line.intersects(rw_boundary):
                        rw_boundary_intersection_points = cur_host_line.intersection(rw_boundary)
                        if rw_boundary_intersection_points.geom_type == 'MultiPoint':
                            for point in rw_boundary_intersection_points.geoms:
                                distance = LineString([point_pos, point]).length
                                if distance < rw_nearest_distance:
                                    rw_nearest_distance = distance
                                    rw_nearest_intersection_point = point
                        elif rw_boundary_intersection_points.geom_type == 'Point':
                            distance = LineString([point_pos, rw_boundary_intersection_points]).length
                            if distance < rw_nearest_distance:
                                rw_nearest_distance = distance
                                rw_nearest_intersection_point = rw_boundary_intersection_points

                # now compare the nearest distance in cloud context with the nearest distance in previous shortest
                if rw_nearest_distance < sensed_shortest_dist:
                    # update global minimum end point and distance
                    ed_points[point_deg] = rw_nearest_intersection_point
                    min_intersection_pt = rw_nearest_intersection_point
                    sensed_shortest_dist = rw_nearest_distance
                    distances[point_deg] = sensed_shortest_dist
    ...
    def ss_reward_Mar_changeskin(self, current_ts, step_reward_record, step_collision_record, xy, full_observable_critic_flag, args, evaluation_by_episode, evaluation_by_fixed_ar):
        ...
        crash_penalty_wall = 50*scale
        reach_target = 50*scale
        dist_to_goal_coeff = 5*scale
        near_building_penalty_coef = 2*scale
        near_drone_penalty_coef = 2*scale
        ...
        for drone_idx, drone_obj in self.all_agents.items():
            ...
            # check whether current action leads to collision with any rw boundaries
            for rw_obj in self.runway_bound:
                conflicting_cloud = polygons_single_cloud_conflict(host_current_circle, rw_obj)
                if drone_obj.reach_target == True:  # when current agent has reached the goal, we don't consider any collision
                    pass
                else:
                    if len(conflicting_cloud) > 0:
                        collide_cloud = 1
                        drone_obj.cloud_collision = True
                        print("{} conflict with runway bound at step {}".format(drone_obj.agent_name, current_ts))
                        break
            ...
            for clound in self.cloud_config:
                ...
                if drone_obj.reach_target == True:  # when current agent has reached the goal, we don't consider any cloud collision
                    ...
                    if len(conflicting_cloud) > 0:
                        collide_cloud = 1
                        drone_obj.cloud_collision = True
                        print("{} conflict with cloud at step {}, essence is {}".format(drone_obj.agent_name, current_ts, clound.agent_essence))
                        break
            ...
            # near_drone_penalty_coef = 3
            # near_drone_penalty_coef = 0
            dist_to_penalty_upperbound = 5  # starting to generate penalty
            dist_to_penalty_lowerbound = 2  # penalty ends as collision happened
            # dist_to_penalty_upperbound = 6
            # dist_to_penalty_lowerbound = 2.5
            # assume when at lowerbound, y = 1
            c_drone = 1 + (dist_to_penalty_lowerbound / (dist_to_penalty_upperbound - dist_to_penalty_lowerbound))
            ...
            # exceed bound condition, don't use current point, use current circle or else will have condition that
            # must use "host_passed_volume", or else, we unable to confirm whether the host's circle is at left or right of the boundary lines
            if (x_left_bound.intersects(host_passed_volume) or
                    x_right_bound.intersects(host_passed_volume) or
                    y_bottom_bound.intersects(host_passed_volume) or
                    y_top_bound.intersects(host_passed_volume)):
                print("drone_{} has crash into boundary at time step {}".format(drone_idx, current_ts))
                drone_obj.bound_collision = True
                rew = rew - crash_penalty_wall
                if args.mode == 'eval' and evaluation_by_episode == False:
            ...
            # # ----------End of termination only during collision to wall on the 3rd time -----------------------
                ...
                # rew = rew + (dist_to_goal-0.01) - near_building_penalty - near_drone_penalty  #4 limit heading change
    ...
    def step(self, actions, current_ts, acc_max, args, evaluation_by_episode, full_observable_critic_flag, evaluation_by_fixed_ar, include_other_AC, use_nearestN_neigh_wRadar, N_neigh):
        ...
        for cloud_agent in self.cloud_config:
            ...
            if len(cloud_agent.preset_traj) > 0:  # for manned aviation only
                cloud_target, cloud_agent = update_cloud_target(cloud_agent, cloud_agent.preset_traj, corridor_radius=cloud_agent.radius, target_radius=0.5)
            else:  # for cloud only
        ...
        for drone_idx_obj, drone_act in zip(self.all_agents.items(), actions):
            ...
            if args.mode == 'eval' and evaluation_by_fixed_ar == True:
                ...
                if current_ts < self.all_agents[drone_idx].ini_eta:
                    ...
                    self.all_agents[
                        drone_idx].eta = None  # once the AC move into the play, we can make eta become None, to signify that it is activated
