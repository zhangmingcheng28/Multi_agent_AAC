...
def main(args):
    ...
    include_other_AC = True  # used for change skin during training, whether include the surrounding ACs in probe.
    # include_other_AC = False

    # save_cur_eva_OD = True  # save all OD information
    ...
    # correspond to 80-150 [km /hour]
    max_spd = 1
    # acc_max = 8
    acc_max = 0.5 # original
    ...
    if args.mode == "eval":
        ...
        # args.max_episodes = 50  # only evaluate one episode during evaluation mode.
        # args.max_episodes = 100
        args.max_episodes = 100
        # args.max_episodes = 250
        # args.max_episodes = 25
        pre_fix = r'D:\MADDPG_2nd_jp\260625_13_50_19\interval_record_eps'
    ...
    while episode < args.max_episodes:  # start of an episode
        ...
        while True:  # start of a step
            if args.mode == "train":
                ...
                for agent_idx, agent in env.all_agents.items():
                    if (agent.eta is None or agent.eta <= step) and not agent.reach_target:
                        activated_agent_per_step = activated_agent_per_step + 1
                if episode not in activated_agent_per_eps:
                    activated_agent_per_eps[episode] = [activated_agent_per_step]
                else:
                    activated_agent_per_eps[episode].append(activated_agent_per_step)
                ...
                # if args.episode_length < step:  # when termination condition reached, without counting drone collision to buildings/wall
                    ...
                    #_____________________evaluation: plot static figures & save_gif______________
                    # if episode % 5 ==0:
                    ...
                    if get_evaluation_status:
                        ...
                        # ---------- new save as gif ----------------------- # evaluation save_gif
                        # else:
                        #     save_gif(env, trajectory_eachPlay, pre_fix, episode, episode)
    ...
    if args.mode == "train":  # only save pickle at end of training to save computational time.
        ...
        with open(path_to_save_eva_OD + '_activated_AC.pickle', 'wb') as handle:
            pickle.dump(activated_agent_per_eps, handle, protocol=pickle.HIGHEST_PROTOCOL)
        if save_cur_eva_OD:
