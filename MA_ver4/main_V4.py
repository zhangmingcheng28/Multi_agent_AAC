...
import csv
import wandb
from parameters_V4 import initialize_parameters
from Multi_Agent_replaybuffer_V4 import MultiAgentReplayBuffer
...
import random
import datetime
from Utilities_V4 import sort_polygons, shapelypoly_to_matpoly, \
...
if __name__ == '__main__':
    ...
    # with open(r'D:\MADDPG_2nd_jp\170523_17_11_02\toplot\all_episode_trajectory.pickle', 'rb') as handle:
    #     all_trajectory = pickle.load(handle)
    #
    # with open(r'D:\MADDPG_2nd_jp\170523_17_11_02\toplot\all_episode_action_taken.pickle', 'rb') as handle:
    #     action_collection = pickle.load(handle)

    # display_trajectory(env, all_trajectory, -1)
    ...
    for i in range(n_episodes):
        ...
        for ts in range(max_t):  # steps inside an episode
            ...
            if len(ReplayBuffer.memory) >= BATCH_SIZE:
                # critic_losses, actor_losses = env.central_learning_v2(ReplayBuffer, BATCH_SIZE, total_agentNum, actor_obs[2])
                critic_losses, actor_losses = env.central_learning(ReplayBuffer, BATCH_SIZE, total_agentNum,
                                                                   actor_obs[2], UPDATE_EVERY)
