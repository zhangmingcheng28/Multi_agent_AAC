...
import numpy as np
import pickle
from grid_env_generation_newframe_randomOD_radar_single_drone_DDPG_changemap_GRU_LSTM_seqLength_reacher import env_generation
from env_simulator_randomOD_radar_single_drone_DDPG_changemap_GRU_LSTM_seqLength_reacher import env_simulator
...
def initialize_parameters():
    ...
    # ---- load preset ------------------
    with open(r'F:\OneDrive_NTU_PhD\OneDrive - Nanyang Technological University\selected_map_v2\bound_allGridPoly.pickle', 'rb') as handle:
        bound_allGridPoly = pickle.load(handle)
    with open(r'F:\OneDrive_NTU_PhD\OneDrive - Nanyang Technological University\selected_map_v2\bound_world_map.pickle', 'rb') as handle:
        bound_world_map = pickle.load(handle)
    with open(r'F:\OneDrive_NTU_PhD\OneDrive - Nanyang Technological University\selected_map_v2\whole_map_polygon.pickle', 'rb') as handle:
        whole_map_polygon = pickle.load(handle)
    with open(r'F:\OneDrive_NTU_PhD\OneDrive - Nanyang Technological University\selected_map_v2\cropped_coord_match_actual_coord.pickle', 'rb') as handle:
