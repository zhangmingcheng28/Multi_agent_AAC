...
def re_arrange_states(batch, no_agents):
    # stack tensors only once
    stacked_elem_0 = torch.stack([elem[0] for elem in batch])
    stacked_elem_1 = torch.stack([elem[1] for elem in batch])
    # use the stacked tensors
    cur_state_list1 = [stacked_elem_0[:, i, :] for i in range(no_agents)]
    cur_state_list2 = [stacked_elem_1[:, i, :] for i in range(no_agents)]
    cur_state_list3 = []

    state_tuple3 = tuple(x[2] for x in batch)
    for i in range(no_agents):
        all_batch_one_agent = []
        for batch_idx, every_agent in enumerate(state_tuple3):
            each_agent_sur_agent = torch.zeros(
                (5 - 1, 6))  # 5 is the maximum agent in the environment
    ...
    obs = [cur_state_list1, cur_state_list2, cur_state_list3]
    return obs
