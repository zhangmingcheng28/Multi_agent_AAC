...
from memory_randomOD_radar_sur_drones_oneModel_att import ReplayMemory, Experience
...
import numpy as np
import torch as T
from utils_randomOD_radar_sur_drones_oneModel_att import device
import csv
...
class MADDPG:
    ...
    def update_myown(self, i_episode, total_step_count, UPDATE_EVERY, single_eps_critic_cal_record, transfer_learning, wandb=None, full_observable_critic_flag=False):
        ...
        # for agent in range(1):
            ...
            if full_observable_critic_flag:
                ...
                actor_loss = - self.critics([stacked_elem_0, stacked_elem_1, stacked_elem_1], ac).mean()
    ...
    def choose_action(self, state, cur_total_step, cur_episode, step, mini_noise_eps, noise_start_level, actor_hiddens, noisy=True):
        ...
        for i in range(self.n_agents):
            ...
            act = self.actors([sb.unsqueeze(0), sb_grid.unsqueeze(0), obs_nei[i].unsqueeze(0)])
            if noisy:
