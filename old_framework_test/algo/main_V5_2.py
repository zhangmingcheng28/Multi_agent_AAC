...
if __name__ == '__main__':
    ...
    for i in range(n_episodes):
        ...
        for ts in range(max_t):  # steps inside an episode
            ...
            reward_aft_action, done_aft_action, check_goal = env.get_step_reward(ts)
        ...
        if len(ReplayBuffer.memory) >= BATCH_SIZE:  # only update when there is enough experience, and only update when a complete episode is done
            ...
            critic_losses, actor_losses = env.central_update(ReplayBuffer, BATCH_SIZE, total_agentNum, actor_obs[2],
                                                             i)  # this is version two of the new framework
